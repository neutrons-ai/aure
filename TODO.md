...
## The call ledger records that a call happened, not what was said

**Where:** [`src/aure/llm/timeout.py`](src/aure/llm/timeout.py) —
`invoke_with_timeout`, beside the existing ledger write; and
[`src/aure/workflow/checkpoints.py`](src/aure/workflow/checkpoints.py) —
`run_info.json`.

**What is wrong.** `llm_calls.jsonl` records `seq`, `timestamp`, `node`,
`model`, `provider`, `duration_s`, `ok`, `error` and the token counts. It does
not record the prompt or the completion. The checkpoints keep the *parsed*
result — `parsed_sample`, `current_model`, the evaluation verdict — so what the
model decided survives a run. What it was asked, and what it actually replied,
do not.

**What it costs.** Measured against the archive in
`aure-validation/results`:

| | |
|---|---|
| completed AuRE runs kept | **165** |
| node checkpoints in them | **5,770** |
| calls recorded in the ledgers | **3,313** |
| exchanges recoverable from any of it | **0** |

Three things that archive cannot be used for, all of which it otherwise could:

- **Re-reading a surprising verdict.** Only the parsed result survives, so
  "why did it propose that layer?" is unanswerable once the run ends — which is
  exactly when the question gets asked.
- **Replaying a run against a different model.** There is no preserved input to
  replay, so comparing a local model against the hosted one means re-running the
  fits rather than re-running the calls.
- **Using the archive as evaluation or training data.** The input has to be
  rebuilt by replaying `prompts.py` over the preceding checkpoint's state.

That reconstruction is not exact, because `run_info.json` records no AuRE
version. `prompts.py` changed during and after the sweep window — the BIC
convention correction in `081e735` being the clearest case, since it altered
what the evaluator was told a good improvement looks like. Rebuilding today's
prompt and pairing it with an August reply pairs the current question with the
previous answer. Dating each sweep to a commit is possible, but that is
archaeology standing in for provenance.

**The change.** Two parts, both small, and the second is the one that stops this
recurring.

1. **Write the exchange at the chokepoint the ledger already uses.** One object
   per call into `llm_trace.jsonl` beside `llm_calls.jsonl`: `seq` (joining to
   the ledger row), `node`, `model`, the system and user messages as sent, and
   the raw completion as received. **Opt-in behind an environment variable** —
   a prompt carries the sample description and the user's hypothesis, which is
   their data and should not land on disk unless asked for.

2. **Record the version in `run_info.json`** — `aure.__version__`, plus the git
   SHA when running from a checkout. A run that says which code produced it can
   be replayed exactly; one that does not can only be dated.

**Why that chokepoint and not the nodes.** `invoke_with_timeout` is where the
ledger already sits, and it is the single point every node's call passes
through. A trace written there cannot miss a call, and cannot be forgotten by a
node added later — which is the property that made the ledger complete and is
the only reason its cost figures are trustworthy.
