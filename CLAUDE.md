...
- CLI entry point: `aure …` (defined in `[project.scripts]` → `aure.cli:main`). Top-level commands: `analyze`, `prepare`, `batch`, `resume`, `checkpoints`, `inspect-checkpoint`, `import-refl1d`, `extract-features`, `serve` / `interactive`, `check-llm`. Full reference is in [README.md](README.md).
