...
- CLI entry point: `aure …` (defined in `[project.scripts]` → `aure.cli:main`). Top-level commands: `analyze`, `prepare`, `batch`, `resume`, `checkpoints`, `inspect-checkpoint`, `import-refl1d`, `serve` / `interactive`, `check-llm`. Full reference is in [README.md](README.md).
...
- `extract_fit_result_from_problem(problem, …)` — moved from `cli.py`; used by the importer.
...
The schema is **states-only**: every analysis declares its files inside a `states:` block, even a single-file analysis (`states: [{name: state0, data_files: [{file: ...}]}]`). There is no top-level `data_file:` / `data_files:` key. The `aure analyze DATA_FILE` CLI positional remains for ad-hoc one-off runs and internally wraps the file in a synthetic `state0`.
